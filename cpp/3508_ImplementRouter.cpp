...
public:
    ...
    {
        if (packetOrder.size() == 0) return std::vector<int>();
        ...
        return std::vector<int>{
            std::get<0>(packetToForward), std::get<1>(packetToForward), std::get<2>(packetToForward)};
    }

    int getCount(int destination, int startTime, int endTime)
    {
        const auto& packetList{packetMap[destination]};
        if (packetList.size() == 0) return 0;
        return std::upper_bound(packetList.begin(), packetList.end(), endTime) -
